...
bool project::is_below(int const chip_id) { return !is_above(chip_id); }
...
void project::unequip(unsigned const char_id, equip::type const type) {
  character_type::equip_type::value_type& eq = character(char_id).equip[type];

  lsd().set_item_num(eq, lsd().item_num(eq + 1));
  eq = 0;
}
...
string project::panorama() {
  return lsd()[111].to_array1d().exists(32)
      ? lsd()[111].to_array1d()[32] : lmu()[31].to<bool>()
      ? lmu()[32] : string();
}
...
void project::character_type::sync() {
  lsd_[51] = int(skill.size());
  lsd_[52].to_binary().assign(skill);

  lsd_[61].to_binary().assign(equip);

  lsd_[81] = int(condition_step_.size());
  lsd_[82].to_binary().assign(condition_step_);
  std::vector<uint16_t> const condition_clean(condition.begin()
                                              , (++std::find(condition.rbegin(), condition.rend(), true)).base());
  ...
  if(prev_lv > next_lv) {
    for(array2d::const_iterator i = skillList.begin(); i != skillList.end(); ++i) {
      ...
      if(current_lv < next_lv) { continue; }
        ...
        std::set<uint16_t>::iterator er = skill.find((*i->second)[2].to<int>());
        if(er != skill.end()) { skill.erase(er); }
      }
    }
    ...
    for(array2d::const_iterator i = skillList.begin(); i != skillList.end(); ++i) {
      ...
      else { skill.insert((*i->second)[2].to<int>()); }
    }
  }
}
